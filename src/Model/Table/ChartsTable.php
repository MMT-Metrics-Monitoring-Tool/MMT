...
class ChartsTable extends Table
{
    ...
    public function reports($project_id, $weekmin, $weekmax, $yearmin, $yearmax) {
        ...
        if ($yearmin != $yearmax) {
            $query = $weeklyreports
                ...
                ->where([
                    ...
                    'OR' => [
                        ['week >=' => $weekmin, 'year' => $yearmin], // min year, weeknum is min or bigger
                        ['week <=' => $weekmax, 'year' => $yearmax] // max year, but weeknumber is max or smaller
                    ]
                ])
                ->toArray();
        ...
        } else { // when looking for reports from a single year
    ...
    public function earnedValueData($project_id, $projectStartDate, $projectEndDate) {
        ...
        // Date function returns a string. Int conversion is for the following loops.
        $xFirstWeek = (int)date('W', strtotime($projectStartDate));
        $xLastWeek = (int)date('W', strtotime($projectEndDate));
    ...
    public function earnedValueData2($project_id, $projectStartDate, $projectEndDate) {
        ...
        // Date function returns a string. Int conversion done for the following loops.
        $xFirstWeek = (int)date('W', strtotime($projectStartDate));
        $xLastWeek = (int)date('W', strtotime($projectEndDate));
    ...
    public function hoursPerWeekData($project_id, $weeklist, $weekmin, $weekmax, $yearmin, $yearmax) {
        ...
        $datemin->setISODate((int)$yearmin, (int)$weekmin);
        $datemax = new Time('midnight');
        $datemax->setISODate((int)$yearmax, (int)$weekmax);
    ...
    // For admin to compare working hours of all the projects. Coach can compare working hours
    // of projects in which he is part of.
    public function hoursComparisonData($weeklist, $weekmin, $weekmax, $yearmin, $yearmax, $admin, $userId) {

        // If user is admin, they can see all the projects.
        if ($admin) {
            ...
            $query2 = $projects
                ...
                ->toArray();
        } else { // Coach can only see total hours of projects what they are part of.
            $members = TableRegistry::get('Members');
            $projectIds = $members
                ->find()
                ->select(['project_id'])
        ...
        // get id and name of each project
        foreach ($query2 as $temp) {
            $temp2 = array();
            $temp2['id'] = $temp->id;
        ...
        foreach ($allProjects as $project) {
            $data = $this->totalhourLineData($project['id'], $weeklist, $weekmin, $weekmax, $yearmin, $yearmax);
        ...
        foreach ($projectRisks as $projectRisk) {
            ...
            foreach ($idlist as $temp) {
                ...
                if (!empty($query)) {
                    ...
                    $severityTemp = $query[0]->severity;
                    $realizationsTemp = $query[0]->realizations;
                }
                
                $probability[] = $probTemp;
                $severity[] = $severityTemp;
                $combined[] = $probTemp * $severityTemp;
                $realizations[] = $realizationsTemp;
            }
            
            $item['probability'] = $probability;
            $item['severity'] = $severity;
            $item['combined'] = $combined;
            $item['total_realizations'] = $projectRisk['realizations'];
            $item['realizations'] = $realizations;
            $data[] = $item;
        }
