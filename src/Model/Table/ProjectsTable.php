...
class ProjectsTable extends Table
{
    ...
    public function validationDefault(Validator $validator): \Cake\Validation\Validator {
        ...
        $validator
            ->allowEmptyString('finished_date');
    ...
    // getAllProjects function returns all the projects. This is used when admin accesses statistics page.
    ...
    // getUserProjects function returns project_id, project_name and finished_date from ProjectsTable
    // for each project. This function is meant to return project information regarding the projects
    // which user is part of.
    public function getUserProjects($projectIds) {
        $ids = array();
    ...
    // Get ids of all the developer and senior developer members
    ...
    public function getRisks($project_id) {
        ...
        if (sizeof($query) > 0) {
            ...
            foreach ($queryR as $temp) {
                if ($temp['probability'] * $temp['severity'] > 15) {
                    $highRisks++;
                }
            }
    ...
    public function getStatusColors($project_id, $metrics) {
        ...
        if ($now <= $endDate) {
            ...
            if (!is_null($earnedValueData)) {
                $CPI = $earnedValueData[6]['CPI'];
                $SPI = $earnedValueData[6]['SPI'];
            }

            $minimumHours = $this->getMinimumHours($project_id);
            $earliestLastSeen = $this->getEarliestLastSeenDate($project_id);

            if (!is_null($earliestLastSeen)) {
                $lastSeen = Time::parseDate($earliestLastSeen);

                // These conditions are the same regardless of the project's phase
                if (date_diff($now, $lastSeen)->format('%a') / 7 > 3) {
                    $statusColors['lastSeen'] = $red;
                } else if (date_diff($now, $lastSeen)->format('%a') / 7 > 2) {
