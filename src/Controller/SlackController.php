...
class SlackController extends AppController
{
    ...
    public function index() {
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            ...
            $slack = $this->Slack->patchEntity($slack, $this->request->getData());
    ...
    public function sendMessage($projectId, $slackData) {
        $result = '';
        $project = $this->Slack->find()->where(['project_id' => $projectId])->first();
        ...
        if ($url != null) {
            ...
            if ($response->code == 200) {
                //This means message is sent successfully
                $result = 'success';
            } else {
                $result = 'fail';
            }
    ...
    public function isAuthorized($user) {
        ...
        if ($this->request->getParam('action') === 'index') {
            //Only admin and senior developer can edit slack information
