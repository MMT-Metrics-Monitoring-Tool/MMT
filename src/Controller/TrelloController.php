...
class TrelloController extends AppController
{
    ...
    public function add() {
        ...
        if ($this->request->is('post')) {
            ...
            $trello = $this->Trello->patchEntity($trello, $this->request->getData());
            $trello['project_id'] = $this->request->getSession()->read('selected_project')['id'];
    ...
    public function isAuthorized($user) {
        ...
        $project_id = $this->request->getSession()->read('selected_project')['id'];
        //Only admin and senior developer can access trello pages
        $allowed = ($project_role == 'senior_developer' || $project_role == 'admin');
        
        if ($allowed) {
            //One can only access trello pages of current project
            if (in_array($this->request->getParam('action'), ['links','edit','delete'])) {
                //For CakePhP 3.4+
