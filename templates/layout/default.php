...
//debug prints
//print_r($this->request->getSession()->read('selected_project_role'));
?>
...
<body <?php if (\Cake\Core\Configure::read('debug')) {
    echo 'class="test-layout"';
      } ?> >
<?php
...
if (!is_null($this->request->getSession()->read('Auth.User'))) {
    ...
    $supervisorquery = \Cake\ORM\TableRegistry::get('Members')->find()
                    ->select(['project_role'])
                    ->where(['user_id =' => $this->request->getSession()->read('Auth.User.id'), 'project_role =' => 'supervisor'])
                    ->first();
    if (!empty($supervisorquery)) {
        ...
        $this->request->getSession()->write('selected_project_role', 'supervisor');
        $supervisor = 1;
    }
}
...
<div id="navgeneral">
    <div class="general-links">
        <ul>
            ...
            <?php
            if (empty(!$this->request->getSession()->read('Auth.User')) && $this->request->getSession()->check('selected_project')) { ?>
...
if (empty(!$this->request->getSession()->read('Auth.User'))) {
    $name = $this->request->getSession()->read('Auth.User.first_name') ?>
            <li class="navBtn" id="dropbtn"><a tabindex="0" role="link">Profile</a>
                
                <div class="dropdown-content">
                    <?php
                    if (!is_null($this->request->getSession()->read('selected_project_role')) && $this->request->getSession()->read('selected_project_role') != 'notmember') { ?>
                        <div id="role"><?php
                            ...
                            echo $current_role[$this->request->getSession()->read('selected_project_role')];
                            ?></div> <?php
                    }
                    ?>
                    <?= $this->Html->link(__('View Profile'), ['controller' => 'Users', 'action' => 'view', $this->request->getSession()->read('Auth.User.id')]) ?>
...
<div id="area51">
    ...
    if (( $this->request->getSession()->read('selected_project') &&
        $this->request->getSession()->read('selected_project')['id'] ) ||
        $supervisor
    ) {
        ...
        if (!($supervisor)) {
            $memid = Cake\ORM\TableRegistry::get('Members')->find()
                        ->select(['id'])
                        ->where(['user_id =' => $userid, 'project_id =' => $this->request->getSession()->read('selected_project')['id']])
                        ->toArray();
        ...
        if (sizeof($memid) > 0) {
            ...
            // supervisors need looping to get notifications with all their member ID's
                ...
                for ($i=0; $i<sizeof($memid); $i++) {
                    $where[] = ['member_id =' => $memid[$i]->id];
                }
                $notifquery = $notifquery->where(['OR' => $where])->toArray();
            }
            ...
            if ($amount = sizeof($notifquery) > 0) { ?>
                <div id="notificationarea">
                    ...
                    <button><span id="label"><?= $this->Html->image('letter.png', array('alt' => 'Image of a letter')); ?></span></button>
    ...
    <div id="topimg">
        <?= $this->Html->image('pitkalogo1.png', array('alt' => 'Tampere university logo')); ?>
    ...
    <nav id="left-title">
        <ul>
            <li class="title-area">
                <?php if (!empty($this->request->getSession()->read('selected_project')['project_name'])) { ?>
                <h1>
                    <?= $project_name = $this->request->getSession()->read('selected_project')['project_name'] ?? '';  ?>
                </h1>
                <?php }?>
            </li>
        </ul>
    ...
    if (!empty($this->request->getSession()->read('Auth.User'))) { ?>
            ...
            if ($this->request->getSession()->check('selected_project')) { ?>
            <nav id="navtop" role="navigation" data-topbar>
            <ul>
                <li id="projectViewBtn" class="navbutton"><?= $this->Html->link(__('Project Info'), ['controller' => 'Projects', 'action' => 'view', $this->request->getSession()->read('selected_project')['id']]) ?></li>
                
                <?php // if not a member, particular links are not shown
                if ($this->request->getSession()->read('selected_project_role') != 'notmember') { ?>
                ...
                <?php
            } else { // end if logged with a project selected
                $admin = $this->request->getSession()->read('is_admin');
                $supervisor = ( $this->request->getSession()->read('selected_project_role') == 'supervisor' ) ? 1 : 0;
                ...
                if ($admin) { ?>
                    ...
                    <!--<li class="navbutton"><?= $this->Html->link('Export metrics', ['controller' => 'metrics', 'action' => 'export']) ?></li>-->
                    <!--<li class="navbutton"><?= $this->Html->link('Export risks', ['controller' => 'weeklyrisks', 'action' => 'export']) ?></li>-->
                    <!--<li class="navbutton"><?= $this->Html->link('Export workinghours', ['controller' => 'workinghours', 'action' => 'export']) ?></li>-->
                <?php }
...
<footer>
    ...
    <div id="area52">
        <div class="footerblock">
            <div id="logoblock">
                <?= $this->Html->image('ylapalkki.jpg', array('alt' => 'University logo Tampereen yliopisto / Tampereen ammattikorkeakoulu')); ?>
                <!--<?= $this->Html->image('pitkalogo1.png', array('alt' => 'Tampere university logo')); ?>
        ...
        <div class="footerblock">
            <ul>
                ...
                <li><?= $this->Html->link(__('Privacy notice'), ['controller' => 'Projects', 'action' => 'privacy']) ?> </li>   
                <li><?= $this->Html->link(__('Accessibility notes'), ['controller' => 'Projects', 'action' => 'accessibilitynotes']) ?> </li>                    
            </ul>
        </div>
</footer>
</body>
