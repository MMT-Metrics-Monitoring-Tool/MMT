...
$(document).on('click','.messagebox .msgaction a.edit',function(){
    ...
    var csrfToken = container.find('.msgaction').data('csrf-token');
    ...
    var input = $('<div style="display:none;"><input type="hidden" name="_csrfToken" autocomplete="off" value="' + csrfToken + '"></div>');
...
// For making data-columns expandable
$(document).on("click", "[data-column]", function () {
    var button = $(this)
    var header = $(button.data("column"))
    var table = header.closest("table")
    var index = header.index() + 1
    var selector = "tbody tr td:nth-child(" + index + ")"
    var column = table.find(selector).add(header)

    button.text(button.text() == "+" ? "-" : "+")
    column.toggleClass("shortened")
});
